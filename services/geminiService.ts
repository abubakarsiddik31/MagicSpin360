...
export const generateSingleImage = async (prompt: string): Promise<string> => {
    ...
    const response: GenerateContentResponse = await ai.models.generateContent({
        ...
        contents: {
            parts: [{ text: `Generate a high-quality image based on this description: "${prompt}". The subject should be centered with a clean or transparent background unless specified otherwise.` }],
        },
        config: {
...
export const enhanceDrawing = async (base64ImageData: string, mimeType: string, prompt: string): Promise<string> => {
    if (!process.env.API_KEY) {
        throw new Error("API_KEY environment variable is not set.");
    }
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
