...
import { editSingleImage, generateSingleImage } from "@/services/gemini";
import { dataUrlToFile, parseDataUrl } from "@/utils/dataUrl";
...
const TEMPLATES = [
  {
    name: "Character",
    prompt:
      "A minimalist sticker of a cute, fluffy red panda, featuring a pastel color palette. The design should have clean lines and soft shading. The background must be transparent.",
  },
  {
    name: "Object",
    prompt:
      "A highly detailed, photorealistic image of a vintage leather-bound book with gold accents, resting on a dark wood table. Soft, warm lighting from a nearby candle.",
  },
  {
    name: "Vehicle",
    prompt:
      "A sleek, futuristic sci-fi sports car, hovering slightly above a neon-lit city street at night. Style should be photorealistic with cinematic lighting.",
  },
  {
    name: "Robot",
    prompt:
      "A friendly, cartoon-style robot with a retro-futuristic design, waving happily. The style is clean and simple, like a modern illustration. Transparent background.",
  },
  {
    name: "Nature",
    prompt:
      "A magical, glowing mushroom in an enchanted forest at night. The style is fantastical and dreamlike, with vibrant, bioluminescent colors.",
  },
];
...
const ImageGenerator: React.FC<ImageGeneratorProps> = ({ onImageReady }) => {
  ...
  const handleEdit = async () => {
    ...
    setError(null);
    try {
      const { base64Data, mimeType } = parseDataUrl(generatedImage);
      const newImageUrl = await editSingleImage(
        base64Data,
        mimeType,
        editPrompt
      );
      ...
      setError(
        e instanceof Error
          ? e.message
          : "An unknown error occurred during edit."
      );
    } finally {
      setEditing(false);
    }
    ...
    <div className="space-y-4">
      <div>
        <p className="text-sm text-gray-400 mb-2">
          Start with a template or write your own prompt.
        </p>
