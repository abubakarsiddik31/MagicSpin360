...
import { enhanceDrawing } from "@/services/gemini";
...
const ImageDrawer: React.FC<ImageDrawerProps> = ({ onImageReady }) => {
  ...
  const handleEnhance = async () => {
    ...
    setError(null);
    try {
      const { base64Data, mimeType } = parseDataUrl(drawing);
    ...
    <div className="space-y-4">
      <div className="relative w-full aspect-square bg-gray-900/50 rounded-lg border border-gray-700">
        ...
        {isViewingEnhanced && !loading ? (
          <img
            ...
            className="absolute inset-0 w-full h-full object-contain rounded-md z-10"
          />
        ) : (
      ...
      <div className="flex space-x-2">
        {isViewingEnhanced ? (
          <button
            ...
            className="w-full text-sm font-semibold py-2 px-4 rounded-md transition-colors bg-gray-600 hover:bg-gray-700"
          >
            ← Edit Drawing
          </button>
        ) : (
          <button
            onClick={handleClear}
            disabled={!hasDrawing}
            className="w-full text-sm font-semibold py-2 px-4 rounded-md transition-colors bg-gray-600 hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Clear Canvas
          </button>
        )}
      ...
      <div className="space-y-2 p-4 border border-gray-700 rounded-lg bg-gray-900/50">
        <p className="text-sm text-gray-400">
          Optionally, describe how to enhance your drawing.
        </p>
        <textarea
          value={prompt}
        ...
        <button
          ...
          disabled={
            loading || !prompt.trim() || !hasDrawing || isViewingEnhanced
          }
